...
import { useSinapiDespesas, SinapiItem } from './useSinapiDespesas';
...
export const useInsumoAnalysis = () => {
  ...
  const extrairPalavrasChave = (texto: string): string[] => {
    ...
    const palavrasComuns = ['de', 'da', 'do', 'para', 'com', 'em', 'a', 'o', 'e'];
  ...
  const encontrarMelhorCorrespondencia = (
    nomeInsumo: string, 
    opcoes: SinapiItem[]
  ): SinapiItem | null => {
    ...
    const opcoesComScore = opcoes.map(opcao => ({
      ...opcao,
      score: calcularSimilaridade(nomeInsumo.toLowerCase(), opcao.descricao.toLowerCase())
    }));
  ...
  const calcularVariacao = (
    ...
    return {
      diferenca,
      percentual,
      tipo: diferenca > 0 ? 'acima' as const : 
            diferenca < 0 ? 'abaixo' as const : 
            'igual' as const
    };
  };
  ...
  const gerarRecomendacoes = (
    ...
    const variacao = calcularVariacao(valorReal, melhorReferencia.preco_unitario);

    if (variacao.tipo === 'acima' && Math.abs(variacao.percentual) > 20) {
      recomendacoes.push(
        `O preço está ${variacao.percentual.toFixed(1)}% acima da referência SINAPI. Considere renegociar com o fornecedor.`
      );
    } else if (variacao.tipo === 'abaixo' && Math.abs(variacao.percentual) > 10) {
      recomendacoes.push(
        `Excelente! O preço está ${Math.abs(variacao.percentual).toFixed(1)}% abaixo da referência SINAPI.`
      );
    } else {
      recomendacoes.push(
