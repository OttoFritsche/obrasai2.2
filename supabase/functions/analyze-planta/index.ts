...
const plantaAnalysisRequestSchema = z.object({
  fileBase64: z.string({
    required_error: "O conteúdo do arquivo (base64) é obrigatório.",
  })
    .min(100, { message: "Conteúdo do arquivo parece inválido ou vazio." }),
  fileName: z.string({ required_error: "O nome do arquivo é obrigatório." })
    .transform((name: string) => name.replace(/[^a-zA-Z0-9_.-]/g, "_")) // Sanitiza o nome do arquivo
    .refine((name: string) => name.length > 0, {
      message: "Nome do arquivo inválido após sanitização.",
    }),
  fileType: z.string({ required_error: "O tipo do arquivo é obrigatório." })
    .refine(
      (type: string) =>
        type.startsWith("application/pdf") || type.startsWith("image/"),
      {
        message: "Tipo de arquivo inválido. Apenas PDF e imagens são aceitos.",
      },
    ),
  obraId: z.string().uuid({ message: "ID da obra inválido" }).nullable()
    .optional(),
});
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: { user: authUser }, error: authError } = await supabaseClient
      .auth.getUser(
    ...
    if (rateLimitData) {
      ...
      if (timeDiff < 60000 && rateLimitData.request_count >= 3) {
        ...
        return new Response(
          JSON.stringify({
            error:
              "Limite de análises excedido. Aguarde 1 minuto entre as análises (máximo 3 por minuto).",
          }),
          {
            status: 429,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (contentType.includes("application/json")) {
      ...
      if (file) {
        ...
        rawInput = {
          ...
          fileName: file.name,
          fileType: file.type,
          obraId: formData.get("obra_id") as string | null,
        };
      }
    } else {
    ...
    // Unificar validação com Zod
    const validationResult = plantaAnalysisRequestSchema.safeParse(rawInput);
    ...
    const { fileBase64, fileName, fileType, obraId } = validationResult.data;
    ...
    // Analisar com GPT-4o
    const analysis = await analyzeFloorPlanWithGPT4o(fileBase64, fileType);
    ...
    const responseData = {
      ...
      analysis_data: {
        summary:
          `Análise de planta baixa com área total de ${analysis.area_total}m²`,
    ...
    if (user && supabaseClient) {
      try {
        ...
        const insertError = metricsInsertError instanceof Error
          ? metricsInsertError
          : new Error(String(metricsInsertError));
...
async function analyzeFloorPlanWithGPT4o(
  base64PDF: string,
  fileType: string,
): Promise<PlantaAnalysis> {
  try {
    console.log("🤖 Iniciando análise com GPT-4o...");
    ...
    console.log(
      "🔑 OpenAI Key status:",
      openaiKey
        ? `Encontrada (${openaiKey.substring(0, 8)}...)`
        : "NÃO ENCONTRADA",
    );
    ...
    if (!openaiKey) {
      ...
      throw new Error(
        "Serviço de análise de plantas temporariamente indisponível. Configuração da API necessária.",
      );
    }
    ...
    if (fileType.includes("pdf")) {
      console.log(
        "⚠️ Arquivo PDF detectado. GPT-4o Vision funciona melhor com imagens.",
      );
      console.log(
        "💡 Para melhores resultados, use uma imagem (JPG/PNG) da planta.",
      );
      ...
      try {
        ...
        const conversionError = e instanceof Error ? e : new Error(String(e));
            ...
            content: [
              {
                type: "text",
                text:
                  `Analise esta planta baixa arquitetônica e extraia as informações solicitadas em formato JSON.
...
Se não conseguir ler a planta claramente, estime valores razoáveis para uma residência típica.`,
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        throw new Error(
          "Arquivo não suportado pela IA. Tente converter o PDF para imagem JPG/PNG.",
        );
      } else if (response.status === 401) {
        throw new Error(
          "Chave da API OpenAI inválida. Serviço temporariamente indisponível.",
        );
      } else if (response.status === 429) {
        throw new Error(
          "Limite de uso da IA excedido. Tente novamente em alguns minutos.",
        );
      } else {
        throw new Error(
          `Erro no serviço de análise: ${response.status} - ${errorText}`,
        );
      }
    }
    ...
    try {
      ...
      if (
        !analysis.area_total || !analysis.comodos ||
        !Array.isArray(analysis.comodos)
      ) {
    ...
    throw new Error(
      `Falha na análise da planta: ${error.message}. Verifique se o arquivo está legível ou tente uma imagem JPG/PNG.`,
    );
  }
}
